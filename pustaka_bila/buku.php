...
switch ($method) {
    ...
    case 'POST':
        ...
        if (!empty($data['judul']) && !empty($data['penerbit']) && !empty($data['pengarang']) && !empty($data['tahun_terbit'])) {
            $stmt = $pdo->prepare("INSERT INTO buku (judul, penerbit, pengarang, tahun_terbit) VALUES (?, ?, ?, ?)");
            $stmt->execute([$data['judul'], $data['penerbit'], $data['pengarang'], $data['tahun_terbit']]);
    ...
    case 'PUT':
        ...
        if ($id) {
            ...
            if ($buku) {
                ...
                $stmt = $pdo->prepare("UPDATE buku SET judul = ?, penerbit = ?, pengarang = ?, tahun_terbit = ? WHERE id = ?");
                $stmt->execute([$judul, $penerbit, $pengarang, $tahun_terbit, $id]);
